...
class Escpos
{
	...
	public function print_invoice($data) {
		...
		$this->printer->text(rtrim($this->columnify($data->invoice_no_prefix, $data->invoice_no, 40, 60, 0,0)));
		$this->printer->feed();
		

		//$this->printer->text(rtrim($this->columnify($invoice_no, $date, 60, 60, 0,0)));
		//$this->printer->feed();
		// & Date
		$date = $data->date_label;
		$date .= ' ' . $data->invoice_date;
		$this->printer->text(rtrim($this->columnify($data->date_label, $data->invoice_date, 40, 60, 0,0)));
		$this->printer->feed();
		$this->printer->text(rtrim($this->columnify($data->sales_person_label, $data->sales_person, 40, 60, 0,0)));
		$this->printer->feed();
		
		$this->printer->text(rtrim($this->columnify($data->customer_label, $data->customer_name, 37, 60, 0,0)));
		$this->printer->feed();
		...
		if (isset($data->lines) && !empty($data->lines)) {
			...
			$this->printer->text($this->drawLine());
			$string = $this->columnify($this->columnify($this->columnify($this->columnify($data->table_product_label ,'',10,60,0,0),$data->table_qty_label, 40,40,0,0), $data->table_unit_price_label, 56,20,0,0), ' '.$data->table_subtotal_label, 73,25,0,0);
			$this->printer->setEmphasis(true);
			...
			foreach ($data->lines as $key => $line) {
				...
				$string = rtrim($this->columnify($this->columnify($this->columnify($product, $quantity,40,60,0,0), $unit_price, 56,20,0,0), $line_total, 76,25,0,0));
		...
		if (isset($data->total_quantity) && !empty($data->total_quantity)) {
			$subtotal = $this->columnify($data->total_quantity_label, number_format((float)$data->total_quantity, 0), 75, 25, 0,0);
		...
		if(!empty($data->payments)){
			...
			foreach ($data->payments as $payment) {
				$total_paid = $this->columnify($payment->method, $payment->amount, 75, 25, 0,0);
				$this->printer->text(rtrim($total_paid));
				$this->printer->feed();
			}
		} else {
			if (isset($data->total_paid) && !empty($data->total_paid)) {
				$total_paid = $this->columnify($data->total_paid_label, $data->total_paid, 75, 25, 0,0);
				//$this->printer->text(rtrim($total_paid));
				//$this->printer->feed();
			}
		}
		...
		if (isset($data->subtotal) && !empty($data->subtotal)) {
			$subtotal = $this->columnify($data->subtotal_label, $data->subtotal, 75, 25, 0,0);
			//$this->printer->text(rtrim($subtotal));
			//$this->printer->feed();
		}
		echo "his222233--";
			...
			$this->printer->setEmphasis(true);
			$total = $this->columnify($data->total_label, $data->total, 75, 25, 0,0);
			$this->printer->text(rtrim($total));
			$this->printer->feed();
			$this->printer->setEmphasis(false);
		}
		if (isset($data->cus_pay_amount) && !empty($data->cus_pay_amount)) { echo $data->cus_pay_amount;
			$subtotal = $this->columnify("Cash Pay: ", number_format((float)$data->cus_pay_amount, 2), 75, 25, 0,0);
			$this->printer->text(rtrim($subtotal));
			$this->printer->feed();
		}
		if (isset($data->cus_pay_balance) && !empty($data->cus_pay_balance)) { echo $data->cus_pay_balance;
			$subtotal = $this->columnify("Balance: ", $data->cus_pay_balance, 75, 25, 0,0);
			$this->printer->text(rtrim($subtotal));
			$this->printer->feed();
		}
		if (isset($data->discount) && !empty($data->discount) && $data->discount != 0) {
			$discount = $this->columnify($data->discount_label, $data->discount, 75, 25, 0,0);
			$this->printer->text(rtrim($discount));
			$this->printer->feed();
		}
		if (isset($data->tax) && !empty($data->tax) && $data->tax != 0) {
			$tax = $this->columnify($data->tax_label, $data->tax, 75, 25, 0,0);
		...
		if (isset($data->total_due) && !empty($data->total_due) && $data->total_due != 0) {
			$total_due = $this->columnify($data->total_due_label, $data->total_due, 75, 25, 0,0);
	...
	{
		$char_per_line = 53;//$this->char_per_line - $remove_for_space;
